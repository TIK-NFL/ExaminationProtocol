...
class ilExaminationProtocolEventInputGUI extends ilExaminationProtocolBaseController
{
    ...
    private function buildEventForm() : void
    {
        ...
        // event input
        $dt_start = $this->field_factory->dateTime($this->plugin->txt("examination_protocol_entry_datetime_start_title"))
            ->withUseTime(true)
            ->withFormat($data_factory->dateFormat()->germanShort())
            ->withValue( $start ?? $this->date_now->format("d.m.Y H:i"))
            ->withRequired(true);
        $dt_end = $this->field_factory->dateTime($this->plugin->txt("examination_protocol_entry_datetime_end_title"))
            ->withUseTime(true)
            ->withFormat($data_factory->dateFormat()->germanShort())
            ->withValue( $end ?? $this->date_now->format("d.m.Y H:i"))
            ->withRequired(true);
        $se_event_type = $this->field_factory->select($this->plugin->txt("examination_protocol_entry_dropdown_event_title"), $this->event_options)
            ->withValue($this->entry['event'] ?? 0);
        $ta_description = $this->field_factory->textarea($this->plugin->txt("examination_protocol_description"))
        ...
        if($this->settings['supervision'] != '2'){
            ...
            $supervisor_options[0] = $this->plugin->txt("examination_protocol_entry_dropdown_supervisor_no_supervisor");

            $se_supervisor = $this->field_factory->select($this->plugin->txt("examination_protocol_entry_dropdown_supervisor_title"), $supervisor_options)
        ...
        if($this->settings['location'] == '0'){
            ...
            $location_options[0] = $this->plugin->txt("examination_protocol_entry_dropdown_location_no_location");

            $se_location = $this->field_factory->select($this->plugin->txt("examination_protocol_entry_dropdown_location_title"), $location_options)
                ->withValue($this->entry['location_id'] ?? 0);
            $event_inputs[] = $se_location;
        }
        $section_input = $this->field_factory->section($event_inputs, $this->plugin->txt("examination_protocol_entry_event_section"));
        ...
        if (empty($_REQUEST['entry_id'])) {
            $this->html = str_replace('<div class="il-standard-form-cmd"><button class="btn btn-default"   data-action="">Save</button>',
                '<div class="il-standard-form-cmd"><button class="btn btn-default"   data-action="">'.$this->plugin->txt("examination_protocol_next").'</button>',
                $this->html);
        }
    }
